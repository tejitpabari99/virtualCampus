...
import {EventCardMobile, EventModal, CustomButton} from '../'
...
class EventsMobile extends React.Component{
    constructor(props) {
        super(props);
        this.state={
            open:false,
            ...
            <div>
                <div >
                    ...
                    {this.state.displayEvents.map((ele, ind) => {
                        if(ele.display) {
                            return(<EventCardMobile key={ind} ele={ele} onClick={() => this.attendEvent(ele)}/>)
                        }
                        return null
                    })}
                </div>
...
export default withStyles(useStyles)(EventsMobile);
