...
let formatDescription = function(desc, eventLink){
    let d = desc + '%0D%0A%0D%0A\n\n';
    for(let i=0; i<eventLink.length; i+=1){
        let add = "";
        let eve = eventLink[i];
        if (eve.hasOwnProperty('title') && eve.hasOwnProperty('link')){
            add += eve.title + " link: " + eve.link + '%0D%0A%0D%0A\n';
        }
        if (eve.hasOwnProperty('pass')){
            add += "Password: " + eve.pass + '%0D%0A\n';
        }
        add += '%0D%0A\n';
        d += add;
    }
...
export default function AddCalendar({info}) {
    ...
    let desc = formatDescription(info.description, info.eventLink);
    ...
    const googleLink = "https://calendar.google.com/calendar/r/eventedit?"
                        ...
                        +"&text="+info.title+"&details="+desc;
    const icGenText = "BEGIN:VCALENDAR\nVERSION:2.0\nBEGIN:VEVENT\nURL:"+window.location.href+"\nDTSTART:"
                        +startTimeFmt+"\nDTEND:"+endTimeFmt+"\nSUMMARY:"
                        +info.title+"\nDESCRIPTION:"+desc
                        +"\nLOCATION:"+loc+"\nEND:VEVENT\nEND:VCALENDAR";
    const icGenName = info.title + "_Calendar.ics";
        ...
        <div className={classes.inline}>
            ...
            <a href={googleLink}
               ...
               title={"Add to Google Calendar"}>
                <img title={"Add to Google Calendar"} className={classes.img} alt={"Add to Google Calendar"}
            ...
            >
                <img  title={"Add to Outlook Calendar"} className={classes.img} alt={"Add to Outlook Calendar"}
