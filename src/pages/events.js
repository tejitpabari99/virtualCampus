...
export function getTimezoneName(loc = this.getCurrentLocationForTimeZone(), dst = this.dst()) {
  ...
  convertEventsTime(event) {
    ...
    makeDisplayEvents(events){
        ...
        for(let i=0; i<events.length; i+=1){
            let ele = events[i];
            ele = this.convertEventsTime(ele);
  ...
  render() {
    ...
    return (
      <Template active={'schedule'}>
        ...
        <Title color={'blue'}>ALL EVENTS {/*(Time Zone: {this.getTimezoneName(this.getCurrentLocationForTimeZone(), this.dst())})*/}</Title>
        <div style={{textAlign:'center'}}>
          <CustomButton href={'/form/'} text={'ADD NEW EVENT'}
